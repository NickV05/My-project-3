...
router.post(
  `/follow/:userProfileId`,
  isAuthenticated,
  async (req, res, next) => {
    try {
      const { userProfileId } = req.params;
      console.log("userProfileId", userProfileId);
      const toBeFollowed = await User.findByIdAndUpdate(
        userProfileId,
        {
...
router.post(
  `/unfollow/:userProfileId`,
  isAuthenticated,
  async (req, res, next) => {
    try {
      const { userProfileId } = req.params;
      console.log("userProfileId", userProfileId);
      const toBeUnFollowed = await User.findByIdAndUpdate(
        userProfileId,
        {
      ...
      const toUnFollow = await User.findByIdAndUpdate(
        req.user._id,
        {
